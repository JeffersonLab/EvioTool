...
//
//  This data is much longer than the test data so takes longer to read.
//
//  EVIO max read speed, using only "read no copy", can read EVIO events at over 500kHz.
//  That is using the C reader only, without copying or parsing anything, just reading the file.
//
...
HPSEvioReader::HPSEvioReader(const char *filename, const char *dictf){
//
  init();
  if(filename){
    open(filename,dictf);
...
HPSEvioReader::~HPSEvioReader(){
  // Destructor and cleanup.
  if(et_evt_ptr) delete et_evt_ptr;
  
}
void HPSEvioReader::init(void){
...
int  HPSEvioReader::getBufSize(void){
...
const unsigned int *HPSEvioReader::getBuffer(void){
  if(get_events_from_et){
    return( et_data );
  }else{
...
int HPSEvioReader::openEv(char *filename){
  int handle;
  string mode="r";
...
#ifdef EVIO_V4_0
int HPSEvioReader::ReadNoCopy(int handle,const unsigned int **buffer,int *buflen){
  return(evReadNoCopy(handle,buffer,buflen));
}
#else
int HPSEvioReader::ReadNoCopy(int handle,const unsigned int **buffer,unsigned int *buflen){
  return(evReadNoCopy(handle,buffer,buflen));
}
#endif


void HPSEvioReader::open(const char *filename,const char *dictf){
...
int HPSEvioReader::openEt(const char *name,int port,int prescale){
  ...
  if (et_open(&et_id, et_file_name.c_str(), openconfig) != ET_OK) {
    printf("HPSEvioReader:: openEt had problems with et_open\n");
    return(1);
  }
  ...
  if ((status = et_station_create(et_id, &et_stat, et_station_name.c_str(), sconfig)) != ET_OK) {
    ...
    if (status == ET_ERROR_EXISTS) {
      ...
      printf("HPSEvioReader::openEt() et_station_create - station %s already exists\n", et_station_name.c_str());
    }
    else if (status == ET_ERROR_TOOMANY) {
      printf("HPSEvioReader::openEt() -- too many stations created\n");
      get_events_from_et=false;
      return(1);
    }
    else {
      printf("HPSEvioReader::openEt() error in station creation\n");
      get_events_from_et=false;
      return(1);
    }
  }
  ...
  if (et_station_attach(et_id, et_stat, &et_attach) != ET_OK) {
    printf("HPSEvioReader::openEt() error in station attach\n");
    return(1);
  }
  ...
  if(et_evt_ptr == NULL){
    cout << "HPSEvioReader::openEt() -- error allocating memory.\n";
    return(1);
  }
...
void HPSEvioReader::parseDictionary(const char *dictf){
...
void HPSEvioReader::printDictionary(){
#ifdef EVIO_V4_0
#else
  if( Dictionary ){
    ...
    if(et_events_remaining ==0){
      if(et_num_read){
        ...
        if (status == ET_ERROR_DEAD) {
          printf("HPSEvioReader::read() -- at dump, ET is dead\n");
          return false;
        }
        else if ((status == ET_ERROR_WRITE) || (status == ET_ERROR_READ)) {
          printf("HPSEvioReader::read() -- at dump, socket communication error\n");
          return false;
        }
        else if (status != ET_OK) {
          printf("HPSEvioReader::ead() - at dump, put error\n");
          return false;
        }
      ...
      if (status == ET_OK) {
        ...
        printf("HPSEvioReader::read() --  ET system is dead\n");
        return false;
      }
      else if (status == ET_ERROR_TIMEOUT) {
        printf("HPSEvioReader::read() -- got timeout\n");
        return false;
      }
      else if (status == ET_ERROR_EMPTY) {
        printf("HPSEvioReader:read() --  no events\n");
        return false;
      }
      else if (status == ET_ERROR_BUSY) {
        printf("HPSEvioReader::read() -- station is busy\n");
        return false;
      }
      else if (status == ET_ERROR_WAKEUP) {
        printf("HPSEvioReader::read() -- someone told me to wake up??\n");
        return false;
      }
      else if ((status == ET_ERROR_WRITE) || (status == ET_ERROR_READ)) {
        printf("HPSEvioReader::read() -- socket communication error\n");
        return false;
      }
      else if (status != ET_OK) {
        printf("HPSEvioReader::read() -- Undetermined error %d\n",status);
        return false;
      }
...
//    et_event_getendian(et_evt_ptr[et_events_remaining -1], &endian);
    ...
    if(swap){
      cout << "HPSEvioReader::read() -- It appears as if this data needs swapping.\n";
    }
    if(getDebug()>1){
...
int HPSEvioReader::parse(EVIO_Event_t *evt, const unsigned int *buff){
...
void HPSEvioReader::Print(void){
  cout << "HPSEvioReader::Print() ";
#ifdef DEBUG
