...
////        indx+=nsample*2;
////
////        fadc.raw_data.push_back(ch);
//        unsigned char chan = GET_CHAR(cbuf,indx);
//        int        nsample = GET_INT(cbuf,indx);
////        FADCchan_raw ch(chan,(unsigned short *)&cbuf[indx],nsample);
////        fadc.raw_data.push_back(ch);
//        fadc.raw_data.emplace_back(chan,(unsigned short *)&cbuf[indx],nsample);
//        indx+=nsample*2;
//
//      }
...
// thus speeding up the code. Turns out that the code below is 3x faster than the code above.
...
std::ostream &operator<<(std::ostream &os, FADCdata const &m) {
  return os << m.Class_Name() << "(" << m.crate << "," << m.slot << ")" ;
}
