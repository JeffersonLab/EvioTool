...
int codanetUdpReceive(unsigned short port, const char *address, int multicast, int *fd)
{
    ...
    printf("codanetUdpReceive, create socket: %d\n",sockfd);

    if (sockfd < 0) {
        if (1 || codanetDebug >= CODA_DEBUG_ERROR) fprintf(stderr, "%sUdpReceive: socket error\n", codanetStr);
        return CODA_SOCKET_ERROR;
    }
    ...
    if (err < 0) {
        if (1|| codanetDebug >= CODA_DEBUG_ERROR) fprintf(stderr, "%sUdpReceive: setsockopt error\n", codanetStr);
        return CODA_SOCKET_ERROR;
    }
    ...
    if (multicast) {
        ...
        ifihead = ifi = codanetGetInterfaceInfo(AF_INET, 0);
        if (ifi == NULL) {
            if (1 || codanetDebug >= CODA_DEBUG_ERROR) {
                fprintf(stderr, "%sGetNetworkInfo: cannot find network interface info\n", codanetStr);
            }
            return CODA_ERROR;
        }
...
/*printf("%sUdpReceive: joining %s on interface %s on port %hu\n", codanetStr, address, ifi->ifi_name, port);*/
    ...
    if (err < 0) {
      ...
      printf("Error in bind for servaddr: %d  %d  %d \n",servaddr.sin_family,servaddr.sin_addr,servaddr.sin_port);
      printf("While trying to bind to: %s \n",address);
        perror(errnostr);
        if (1 || codanetDebug >= CODA_DEBUG_ERROR) fprintf(stderr, "%sUdpReceive: bind error\n", codanetStr);
        return CODA_SOCKET_ERROR;
    }
