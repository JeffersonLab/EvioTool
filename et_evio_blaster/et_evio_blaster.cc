...
int main(int argc,char **argv)
{
  ...
  time1 = 1000L*t1.tv_sec + t1.tv_nsec/1000000L; /* milliseconds */
   ...
   int i_file=-1;
   while(true){
      if(++i_file >= args.filenames.size()){
         cout << "=======================================\n";
         cout << "====  RECYCLING TO THE FIRST FILE  ====\n";
         cout << "=======================================\n";
         i_file = 0;
      }
      ...
      while (evio_has_events) {
         ...
         for (int i=0; i < numRead; i++) {
            ...
            //      if( (status=evReadAlloc(fEvioHandle,&evio_ptr,&fEvioBufLen)) != S_SUCCESS){
            ...
            if(evio_buflen + 16 > args.et_event_size_max){
               ...
               //        unsigned int *tmp_ptr = (pdata + 8 + fEvioBufLen);
      ...
      time2 = 1000L*t2.tv_sec + t2.tv_nsec/1000000L; /* milliseconds */
         ...
         if (time > 1000) {
            ...
            if(args.debug) printf("%s: %7.4g kHz,  %7.4g kHz Avg.\n", argv[0], rate/1000., avgRate/1000.);
            count = 0;
#if defined __APPLE__
            gettimeofday(&t1, NULL);
...
void Print_Usage(const char *name){
   ...
   cout << "  -d  -delqy us      Delay after each event. [0]\n";
