...
{
  int             i,ii, j, c, i_tmp, status, swappedData, numRead;
  ...
  while (1) {
    ...
    if (1) {
      unsigned int *pdata;
      for (i=0; i < numRead; i++) {
        swappedData = i + startingVal; // ET_SWAP32(i + startingVal);
        et_event_getdata(pe[i], (void **) &pdata);
        //memcpy((void *)pdata, (const void *) &swappedData, sizeof(int));
        pdata[0] = swappedData;
        pdata[1] = 1001*startingVal;
        pdata[2] = 123456;
        pdata[3] = 111111;
        // et_event_setendian(pe[i], ET_ENDIAN_NOTLOCAL);
        et_event_setendian(pe[i], ET_ENDIAN_LOCAL);
        et_event_setlength(pe[i], 5*sizeof(int));
        et_event_setcontrol(pe[i], control, sizeof(control)/sizeof(int));
        //printf("Wrote:  %10d  %10d  %10d  %10d\n", pdata[0],pdata[1],pdata[2],pdata[3]);
        int *tmp_pdata=(int *)pdata;
        printf("pdata (%5d) :",5);
        for(ii=0;ii<5;ii++){
